...
namespace Assembler
{
    ...
    void preProcessExpression(const std::vector<std::string>& tokens, int tokenIndex, std::string& input, bool stripWhiteSpace)
    {
        ...
        // Pre-process
        for(size_t j=tokenIndex; j<tokens.size(); j++)
        {
            // Strip comments
    ...
    {
        bool modified = false;
        for(size_t i=0; i<equates.size(); i++)
        {
            for(;;)
            {
    ...
    {
        bool success = false;
        for(size_t i=0; i<_equates.size(); i++)
        {
    ...
    bool handleDefineByte(const std::vector<std::string>& tokens, int tokenIndex, const Instruction& instruction, bool createInstruction, int& dbSize)
    {
        ...
        if(quotes)
        {
            ...
            {
                for(size_t j=1; j<token.size(); j++) // First instruction was created by callee
                {
        ...
        for(size_t i=tokenIndex+1; i<tokens.size(); i++)
        {
                ...
                for(size_t t=0; t<tokens.size(); t++)
                {
                    if(tokens[t] == macro._name)
                    {
                        ...
                        if(tokens.size() - t > macro._params.size())
                        {
                            ...
                            for(size_t ml=0; ml<macro._lines.size(); ml++)
                            {
                                // Tokenise macro line
                                ...
                                // Replace parameters
                                for(size_t mt=0; mt<mtokens.size(); mt++)
                                {
                                    for(size_t p=0; p<macro._params.size(); p++)
                                    {
    ...
    bool handleMacroStart(const std::string& filename, const LineToken& lineToken, const std::vector<std::string>& tokens, Macro& macro, int adjustedLineIndex)
    {
        ...
        // Save params
        for(size_t i=2; i<tokens.size(); i++) macro._params.push_back(tokens[i]);
    ...
    {
        for(size_t i = 0; i<_gprintfs.size(); i++)
        {
            for(size_t j = 0; j<_gprintfs[i]._vars.size(); j++)
            {
                bool success = false;
    ...
    bool getGprintfString(int index, std::string& gstring)
    {
        ...
        size_t subIndex = 0;
        for(size_t i=0; i<gprintf._vars.size(); i++)
        {
            char token[256];
                            ...
                            else if(opcodeType == vCpu  &&  opcode == 0xCF)
                            {
                                ...
                                if(_callTablePtr  &&  operand != 0x18)
                                {
                                    ...
                                    if(evaluateLabelOperand(tokens, tokenIndex, label, false))
                                    {
                                        ...
                                        for(size_t i=0; i<_callTableEntries.size(); i++)
                                        {
